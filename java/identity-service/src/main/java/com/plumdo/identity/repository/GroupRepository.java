...
public interface GroupRepository extends BaseRepository<Group, Integer> {
	@Query("select a from Group a, UserGroup b where a.id = b.groupId and a.status="+TableConstant.ROLE_STATUS_NORMAL+" and b.userId = ?1 ")
	List<Group> findByUserId(int userId);

	List<Group> findByParentId(int parentId);

	List<Group> findByStatus(byte status);
}