...
public class BaseTaskResource {
	...
	protected void checkTaskAuthorize(TaskInfo task, boolean isFinished) {
		/*
		 * if(!BasicAuthenticationProvider.Admin.equals(Authentication.
		 * getAuthenticatedUserId())){ if(task.getAssignee()!=null){
		 * if(!task.getAssignee().equals(Authentication.getAuthenticatedUserId())){
		 * throw new FlowableConflictException("task id " + task.getId() +
		 * " the assignee "+task.getAssignee()+" is not you."); } }else{
		 * if(!isFinished){ boolean candidateIsNull = true; List<IdentityLink>
		 ...
		 * candidateIsNull = false; break; } } if(!candidateIsNull){
		 * if(taskService.createTaskQuery().taskId(task.getId()).taskCandidateUser(
		 * Authentication.getAuthenticatedUserId()).count()==0){ throw new
		 * FlowableConflictException("task id " + task.getId() +
		 * " the candidate user is not include you."); } } } } }
		 */
	}
}
