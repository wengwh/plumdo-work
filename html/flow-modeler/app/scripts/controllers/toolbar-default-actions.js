...
FLOWABLE.TOOLBAR = {
	ACTIONS : {
		...
		/**
		 * Helper method: fetches the Oryx Edit plugin from the provided scope, if
		 * not on the scope, it is created and put on the scope for further use.
		 * 
		 * It's important to reuse the same EditPlugin while the same scope is
		 * active, as the clipboard is stored for the whole lifetime of the scope.
		 */
		...
		/**
		 * Helper method: fetches the Oryx View plugin from the provided scope, if
		 * not on the scope, it is created and put on the scope for further use.
		 */
...
angular.module('flowableModeler').controller('SaveModelCtrl', [ '$rootScope', '$scope', '$http', '$route', '$location', 'editorManager', function($rootScope, $scope, $http, $route, $location, editorManager) {
	...
	$scope.okClicked = function() {
		if ($scope.error) {
			if ($scope.error.conflictResolveAction === 'discardChanges') {
				$scope.close();
				$route.reload();
			} else if ($scope.error.conflictResolveAction === 'overwrite' || $scope.error.conflictResolveAction === 'newVersion') {
				$scope.save();
...
angular.module('flowableModeler').controller('ValidateModelCtrl', [ '$scope', '$http', 'editorManager', function($scope, $http, editorManager) {
	...
	$scope.errorGrid = {
		...
		columnDefs : [ {
			...
			cellTemplate : 'views/properties/errorgrid-critical.html',
			width : 100
		} ]
	};
